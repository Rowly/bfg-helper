...
async function resolveSelectedAgainstShip(selected, target, selectedMarker) {
  ...
  return openActionResolution({
    ...
    roll: async () => {
      ...
      const brace = targetCombat.hulk ? { dice: [], saved: 0, unsaved: hits } : await rollBraceSaves(target, hits, "Brace saves against attack craft");
      const critical = targetCombat.hulk ? { after: getCriticalState(target), escortDestroyed: false, extraDamage: 0, results: [] } : await rollCriticalHits(target, brace.unsaved);
      const remainingHull = targetCombat.hulk ? 0 : critical.escortDestroyed ? 0 : Math.max(0, targetCombat.currentHits - brace.unsaved - critical.extraDamage);
    ...
    apply: async outcome => {
      ...
      await ChatMessage.create({ speaker: ChatMessage.getSpeaker({ token: selected[0].document }), content: `<div class="bfg-dice-chat-result"><strong>${waveName} against ${foundry.utils.escapeHTML(target.name)}</strong><br>CAP interceptions: ${outcome.intercepted.length}; turret rolls: ${outcome.turretResults.join(", ") || "None"}; turret kills: ${outcome.turretVictims.length}<br>Surviving bombers: ${outcome.survivingBombers.length}; surviving boarding craft: ${outcome.survivingAssault.length}; fighter suppression attacks: ${outcome.suppressionFighters}<br>Bomber attack runs: ${outcome.bomberAttackCounts.join(", ") || "None"}; bomber attack dice: ${outcome.attackResults.join(", ") || "None"}; Armour: ${targetNumber}+; hits: ${outcome.hits}; shields ignored<br>Brace saves: ${outcome.brace.dice.join(", ") || "None"}; damage saved: ${outcome.brace.saved}<br>Critical checks: ${outcome.criticalChecks}; critical effects: ${foundry.utils.escapeHTML(outcome.criticalEffects)}; critical damage: ${outcome.critical.extraDamage ?? 0}<br>Catastrophic result: ${foundry.utils.escapeHTML(outcome.catastrophicResult)}; remaining hull: ${outcome.remainingHull}<br>Pending boarding-craft Hit-and-Run attacks: ${outcome.pendingHitAndRun}</div>` });
    }
  });
}
